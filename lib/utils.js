...
function normalizeBinary (binaryPath, platform, arch) {
  return when.promise(function(resolve, reject) {
    ...
    function fallBack () {
      resolve(path.join(process.env["ProgramFiles"], appName, "firefox.exe"));
    }

    var rootKey = path.join('\\Software\\Mozilla\\', appName);
    ...
    return when.promise(function(resolve, reject) {
      ...
      versionKey.get("CurrentVersion", function(err, key) {
        return (err) ? reject(new Error("only firefox x64 is supported on Windows x64")) : resolve(key.value);
      });
